...
class HDF5Dataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, list]:
        ...
        if(f'{self.pde}' == 'CE'):
            ...
            u_super = self.data[self.dataset_super][idx][::self.ratio_nt][None, None, ...].astype(np.float32)
            left = u_super[..., -3:-1]
            right = u_super[..., 1:3]
            u_super_padded = torch.tensor(np.concatenate((left, u_super, right), -1)).float()
            weights = torch.tensor([[[[0.2]*5]]]).float()
            u_super = F.conv2d(u_super_padded, weights, stride=(1, self.ratio_nx)).squeeze().numpy().astype(np.float32)
            x = self.x

            # Base resolution trajectories (numerical baseline) and equation specific parameters
            u_base = self.data[self.dataset_base][idx].astype(np.float32)
            variables = {}
...
def plot_traj(traj, ax=None, title=""):
    '''
...
def plot_im(u, ax = None, fig=None, title='test', aspect='auto'):
...
def vis_2d(u, title=""):
    ...
    for i in range(u.shape[0]):
        im = ax.imshow(u[i].squeeze(), animated=True, cmap='seismic')
        ims.append([im])
